...
blogFunctions.getAllPost = async function () {
	try {
		const results = await sql_query(`
			...
			from ${process.env.DB_DATABASE}.post order by p_view desc;
		`);
...
blogFunctions.updatePost = async function (updateData) {
	...
	try {
		...
		`);
		if (updateData.uploadImg.length) {
			for (const img of updateData.uploadImg) {
				await sql_query(`
					insert into ${process.env.DB_DATABASE}.file (P_ID, F_IMG) VALUE (${updateData.id}, '${img}');
				`);
			}
		}
		if (updateData.deleteImg.length) {
			for (const img of updateData.deleteImg) {
				await sql_query(`   
					delete from ${process.env.DB_DATABASE}.file where F_IMG = '${img.F_IMG}';
				`);
				const name = img.F_IMG.split("com/")[1];
				params.Key = name;
...
blogFunctions.deletePost = async function (id) {
	...
	try {
		...
		if (deleteInfo.length) {
			const deleteFiles = deleteInfo.map(
				(delFile) => delFile.F_IMG.split("com/")[1]
			);
